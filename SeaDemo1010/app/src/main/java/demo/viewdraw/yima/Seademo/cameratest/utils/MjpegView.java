...
public class MjpegView extends SurfaceView implements SurfaceHolder.Callback {
	...
	private Paint overlayPaint;						//用于fps涂层绘画笔
	private Paint suBitmap;
	private Paint stopBitmap;
	...
	/**
	 * 因为在res/layout目录下的 activity_video.xml中作为自定义的控件使用了这个类，所以需要给此类提供带有属性形参的构造函数
	 * 当在 VideoActivity 通过ID找到这自定义的控件时，该构造函数将被调用，所以将该构造函数设为public
	 * @param context
	 * @param attrs
	 */
	...
	/**
	 * setFps，getFps，setServer source都在MaiActivity使用
	 * @param b
	 */
	...
	public void getshipsd(double ship_sd,double pre_leida,double stopship_dis) {
		sd = ship_sd;
		ds = pre_leida;
		ssd = stopship_dis;
	}
	...
	public class MjpegViewThread extends Thread {
		...
		private Bitmap stopBitmap = null;
		private Bitmap mjpegBitmap = null;
		private Bitmap suBitmap=null;
	...
	//		String fps = "";
			while (bRun) {
				if (bsurfaceIsCreate) {
					...
					try {
						...
						//同步图像的宽高设置
						synchronized (mSurfaceHolder) {
							destRect = destRect(mjpegBitmap.getWidth(), mjpegBitmap.getHeight());
						}

						//当主activity点击相册和设置跳转时，Surfaceview被销毁，此时canvas将为空
						//Log.e(">>>", "canvas="+canvas);
						if(canvas != null){
							canvas.drawPaint(new Paint());
							canvas.drawBitmap(mjpegBitmap, null, destRect, p);
							if (bIsShowFps){
								calculateFps(destRect, canvas, p);
							}
		...
		public void calculateFps(Rect destRect, Canvas c, Paint p) {
			...
			if (suBitmap != null) {
				 ...
				 * 然后调用画布的绘制位图方法绘图
				 */
				height = ((ovlPos & 1) == 1) ? destRect.top+40
						: destRect.bottom - suBitmap.getHeight()+40;
				width = ((ovlPos & 8) == 8) ? destRect.left
						: destRect.right - suBitmap.getWidth();
				c.drawBitmap(suBitmap, width, height, null);
			}
			...
			if (stopBitmap != null) {
				 ...
				 * 然后调用画布的绘制位图方法绘图
				 */
				height = ((ovlPos & 1) == 1) ? destRect.top+80
						: destRect.bottom - stopBitmap.getHeight()+80;
				width = ((ovlPos & 8) == 8) ? destRect.left
						: destRect.right - stopBitmap.getWidth();
				c.drawBitmap(stopBitmap, width, height, null);
			}
			...
			if ((System.currentTimeMillis() - start) >= 1000) {
				...
				suBitmap = makeFpsOverlay(overlayPaint, sds);
				stopBitmap = makeFpsOverlay(overlayPaint, asd);
				frameCounter = 0;				
			}
		}
